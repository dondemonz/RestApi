...
def test_DeleteV1TaskCode200():
    ...
    # print(task_id)
    time.sleep(3)
    response = requests.delete(url="http://" + slave_ip + ":8888/api/v1/export/tasks/"+task_id, headers=headers, data=json.dumps(dict(data)), auth=auth)
    user_resp_code = "200"
...
def test_DeleteV1TaskCode401():
    response = requests.delete(url="http://" + slave_ip + ":8888/api/v1/export/tasks/123", auth=("", ""))
    user_resp_code = "401"
    assert str(response.status_code) == user_resp_code
