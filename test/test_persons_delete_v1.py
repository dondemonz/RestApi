...
def test_DeleteV1PersonsCode200(fix):
    ...
    time.sleep(3)
    response = requests.delete(url="http://" + slave_ip + ":8888/api/v1/persons/1.888", auth=auth)
    user_resp_code = "200"
...
def test_DeleteV1PersonsCode401():
    response = requests.delete(url="http://" + slave_ip + ":8888/api/v1/persons/999.0", auth=("", ""))
    user_resp_code = "401"
    assert str(response.status_code) == user_resp_code

def test_DeleteV1PersonsCode404():
    data = "Person 0 not found."
    response = requests.delete(url="http://" + slave_ip + ":8888/api/v1/persons/0", auth=auth)
    user_resp_code = "404"
    assert str(response.status_code) == user_resp_code
    body = json.dumps(response.json())
    data1 = json.loads(body)
