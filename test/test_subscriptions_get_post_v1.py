...
def test_GetV1SubscriptionByIdCode200And401AndPostV1SubscriptionCode201AndSendPostCallback(fix):
    ...
    post_response = requests.post(url="http://" + slave_ip + ":"+restPort+"/api/v1/events/subscriptions", headers=headers, data=json.dumps(dict(data)), auth=auth)
    user_resp_code = "201"
    ...
    response = requests.get(url="http://" + slave_ip + ":"+restPort+"/api/v1/events/subscriptions/"+subscription_id, auth=auth)
    user_resp_code = "200"
    ...
    response1 = requests.get(url="http://" + slave_ip + ":"+restPort+"/api/v1/events/subscriptions/" + subscription_id, auth=("", ""))
    user_resp_code = "401"
    assert str(response1.status_code) == user_resp_code

def test_PostV1SubscriptionCode401():
    response = requests.post(url="http://" + slave_ip + ":"+restPort+"/api/v1/events/subscriptions", auth=("", ""))
    user_resp_code = "401"
    assert str(response.status_code) == user_resp_code

def test_GetV1SubscriptionsByIdCode404():
    data = "Unknown Task id:0"
    response = requests.get(url="http://" + slave_ip + ":"+restPort+"/api/v1/events/subscriptions/0", auth=auth)
    user_resp_code = "404"
    assert str(response.status_code) == user_resp_code
    body = json.dumps(response.json())
    data1 = json.loads(body)
...
def test_GetV1SubscriptionsCode200():
    data = "success"
    response = requests.get(url="http://" + slave_ip + ":"+restPort+"/api/v1/events/subscriptions/", auth=auth)
    user_resp_code = "200"
    assert str(response.status_code) == user_resp_code
    body = json.dumps(response.json())
    data1 = json.loads(body)
...
def test_GetV1SubscriptionsCode401():
    response = requests.get(url="http://" + slave_ip + ":"+restPort+"/api/v1/events/subscriptions/", auth=("", ""))
    user_resp_code = "401"
    assert str(response.status_code) == user_resp_code
