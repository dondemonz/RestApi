...
def test_GetV1TasksCode200():
    data = "success"
    response = requests.get(url="http://" + slave_ip + ":"+restPort+"/api/v1/export/tasks", auth=auth)
    user_resp_code = "200"
...
def test_GetV1TasksCode401():
    response = requests.get(url="http://" + slave_ip + ":"+restPort+"/api/v1/export/tasks", auth=("", ""))
    user_resp_code = "401"
    assert str(response.status_code) == user_resp_code
