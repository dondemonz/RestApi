...
def test_PutV1SubscriptionsCode200And401AndSendPostCallback(fix):
    ...
    response = requests.put(url="http://" + slave_ip + ":"+restPort+"/api/v1/events/subscriptions/"+subscription_id, headers=headers, data=json.dumps(dict(data1)), auth=auth)
    user_resp_code = "200"
    ...
    response1 = requests.put(url="http://" + slave_ip + ":"+restPort+"/api/v1/events/subscriptions/" + subscription_id, auth=("", ""))
    user_resp_code = "401"
...
def test_PutV1SubscriptionsCode404():
    ...
    response = requests.put(url="http://" + slave_ip + ":"+restPort+"/api/v1/events/subscriptions/0", headers=headers, data=json.dumps(dict(data1)), auth=auth)
    user_resp_code = "404"
    assert str(response.status_code) == user_resp_code
    body = json.dumps(response.json())
    data1 = json.loads(body)
