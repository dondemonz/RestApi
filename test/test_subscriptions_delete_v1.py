...
def test_DeleteV1SubscriptionsCode200():
    ...
    response = requests.delete(url="http://" + slave_ip + ":8888/api/v1/events/subscriptions/"+subscriptionId, auth=auth)
    user_resp_code = "200"
...
def test_DeleteV1SubscriptionsCode401():
    ...
    response = requests.delete(url="http://" + slave_ip + ":8888/api/v1/events/subscriptions/"+subscriptionId, auth=("", ""))
    user_resp_code = "401"
    assert str(response.status_code) == user_resp_code

def test_DeleteV1SubscriptionsCode404():
    data = "Unknown Subscription id:0"
    response = requests.delete(url="http://" + slave_ip + ":8888/api/v1/events/subscriptions/0", auth=auth)
    user_resp_code = "404"
    assert str(response.status_code) == user_resp_code
    body = json.dumps(response.json())
    data1 = json.loads(body)
